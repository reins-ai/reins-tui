...
/**
 * SidebarContent renders the sidebar internals without the outer
 * border/width container. Designed to be placed inside a DrawerPanel
 * for the summoned panel layout.
 */
export function SidebarContent({ isFocused }: SidebarContentProps) {
  const { state, dispatch } = useApp();
  const { tokens } = useThemeTokens();
  const conversations = useConversations();
  const filteredConversations = useMemo(() => conversations.filteredConversations(), [conversations]);
  ...
  return (
    <Box style={{ flexDirection: "column", flexGrow: 1 }}>
      ...
      <Text
        content={
          isFocused
            ? "↑↓: select  Enter: open  R: rename  D: delete"
            : "Ctrl+1 to focus"
        }
        style={{ color: tokens["text.muted"] }}
      />
